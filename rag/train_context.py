...
def add_to_vectorstore(chunks: List[Document]):
    print(f"[INFO] Preparing to add chunks to Chroma vectorstore at '{DB_PATH}'...")
    ...
    try:
        ...
        print("[WARN] Could not fetch existing IDs, assuming empty DB.")
        existing_ids = set()

    print(f"[INFO] Existing IDs in DB: {len(existing_ids)}")
    ...
    if new_chunks:
        ...
        print(f"[SUCCESS] ✅ {len(new_chunks)} new chunks added to Chroma.")
    else:
...
def main():
    ...
    print("[STEP 4] Embedding and storing in Chroma...")
